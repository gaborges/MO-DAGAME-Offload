...
public class AlgorithmsStudy extends org.ufrgs.custom.Experiment {
    ...
    public AlgorithmsStudy(List<FeatureModelBundle> featureModels, int baseAlgorithm,
        ...
        setmFeatureModels(featureModels);
        ...
        setIsConditionsToOffloadSatisfied(false);
    }
    ...
    public void algorithmSettings(String problemName, int problemIndex, Algorithm[] algorithm)
        ...
        try {
            ...
            FeatureModel fm = getmFeatureModels().get(problemIndex).getFeatureModel();
            Configuration seed = getmFeatureModels().get(problemIndex).getSeed();
    ...
    public void run() throws JMException, IOException {
        int featureModelsCount = getmFeatureModels().size();
        problemList_ = new String[featureModelsCount];
        for (int i = 0; i < featureModelsCount; i++) {
            String name = getmFeatureModels().get(i).getFeatureModel().getName();
        ...
        // add offload particular parameters
        map_.put("maxEvaluations",evaluations_);
        map_.put("populationSize",populationSize_);
        map_.put("algorithm_", algorithm_);
